...
**Paso uno: congelar el objetivo.** Cuando corre `review-start`, lo primero que hace es construir un **snapshot** inmutable y direccionado por contenido de lo que se va a revisar: la lista exacta de archivos con sus hashes, incluyendo los untracked intencionales, los archivos nuevos que son parte del cambio pero todavia no estan en el index de git. Despues agrega un evento genesis que abre la transaccion. Desde este momento, revisas una foto congelada, no un blanco en movimiento. Por que importa tanto? Porque la falla clasica del review con agentes es que el codigo cambia DEBAJO del review: el revisor lee el archivo A, mientras tanto el fixer edita el archivo B, y la aprobacion final cubre una quimera que nunca existio completa. El snapshot mata eso. Todos en la transaccion revisan los mismos bytes congelados, identificados por hash.
...
**Paso cinco: como maximo UNA ronda de fixes.** Los fixes son unidades de trabajo atomicas, cada una con su evidencia de tests y su frontera de rollback. Despues, UN validador de fix-delta con alcance acotado re-chequea solo las lineas tocadas por los fixes, y tiene exactamente dos salidas: aprobar o escalar. No puede pedir otra ronda. Te acordas del test acido del Capitulo 20, acotado y auditable? Esto es acotado, soldado a la estructura. El loop no puede girar porque no hay loop que girar.

Por que el validador ve SOLO las lineas tocadas por los fixes? Prevencion de scope creep, y corta en las dos direcciones. Direccion uno, el fixer: con un validador de diff completo, un agente de fixes puede contrabandear "mejoras" mas alla de los hallazgos confirmados, refactors que nadie pidio, y el validador no puede decir "esto esta fuera de alcance" porque todo esta en alcance. Acotado al delta, cualquier linea cambiada fuera del radio de los hallazgos se ve como lo que es: cambio no autorizado. Direccion dos, el validador mismo: un validador que re-lee el diff completo va a ENCONTRAR COSAS NUEVAS, es su naturaleza. Hallazgos nuevos sobre lineas viejas significan otra ronda de fixes, otra validacion, mas hallazgos nuevos. Un loop sin cota disfrazado de calidad. Acotar al delta hace la convergencia estructural: el fix achica la lista de hallazgos abiertos, la validacion chequea el achique, nada puede crecer. Si algo en una linea no tocada esta realmente roto, estaba roto antes de este review, y lo atrapa el proximo review del proximo cambio. Este se cierra.

**Paso seis: verificacion final independiente.** Un verificador fresco chequea la transaccion completa: requisitos cumplidos, evidencia actual de tests y build, cada hallazgo del ledger resuelto o refutado explicitamente, identidad del snapshot intacta, contadores coherentes. Despues, uno de exactamente dos estados terminales: **approved**, que emite el recibo atado al contenido, o **escalated**, que emite una denegacion legible por maquina para un humano. No existe un tercer final. Nada de "basicamente listo", nada de "aprobado a ojo". Dos puertas.

```text
snapshot --> lentes (0|1|4, solo lectura, una pasada cada uno) --> congelar ledger
    --> ruteo por evidencia (deterministica | inferencial | insuficiente)
    --> maximo 1 ronda de fixes --> validacion de fix-delta acotada
    --> verificacion final independiente --> approved | escalated
```
...
| `judges_confirmed`          | Todos los resultados de lentes estan; nada se agrega tarde       |
...
| `fixing`                    | La unica ronda de fixes: unidades atomicas, fronteras de rollback |
| `fix_validating`            | Validador de delta acotado; aprobar o escalar, nada mas          |
...
Doblemos el capitulo en una sola oracion, el disenio en su forma mas pura: **los agentes y los espejos pueden decir lo que quieran, el gate solo cree lo que puede re-derivar del store append-only y de los bytes actuales del repositorio.** Leela de nuevo, porque cada seccion que caminamos vive adentro de ella. Los recibos atados al contenido por hash. El ciclo de vida congelado en snapshots y ledgers. La maquina de 12 estados con los presupuestos soldados adentro. La cadena que solo crece hacia adelante. Los espejos que nunca le ganan a la fuente. Los bundles tratados como carga no confiable. Los gates que recalculan en lugar de aceptar. Los dos bugs reales que el sistema atrapo en su propio pull request, porque una verificacion que no se aplica a si misma es marketing.
...
- Como maximo una ronda de fixes; el validador de delta solo puede aprobar o escalar.
...
- **gentle-ai** (github.com/Gentleman-Programming/gentle-ai): el stack open source de agentes que este capitulo disecciono. El PR #1093 es la feature de bounded review transactions en si: el constructor de snapshots, la maquina de 12 estados, el store append-only, los gates y el ledger donde viven los dos bugs reales.
