...
The current ordinary review path in Gentle AI v2.1.2 is five actions, or four when risk selection chooses no lens:
...
That diagram is the successful correction path, not a claim that every failed attempt is impossible. v2.1.2 keeps failed targeted corrections inside the same correction transaction, with cumulative accounting and a maximum of three failed attempts. We will open that edge after the happy path.
...
Three failed compact correction attempts exhaust the lineage even if their measured delta is zero. Exceeding cumulative changed lines also escalates. This is not "three correction rounds" in the old sense, where each round could reopen review and discover fresh scope. It is one correction transaction with up to three bounded attempts to satisfy the same frozen causal findings. That distinction matters.
